...
public:
    ...
    Vertex getMax(vector<Vertex> vert);
};
...
Vertex Vertex::getMax(vector<Vertex> arreglo){
    ...
    //Algoritmo para ordenar con el metodo de intercambio de menor a mayor
    for (int i = 0; i < arreglo.size(); i++){
        for (int j = 0; j < arreglo.size(); j++){
            if (arreglo.at(i).getSize()<arreglo.at(j).getSize()){
                aux=arreglo.at(i);
                arreglo.at(i)=arreglo.at(j);
                arreglo.at(j)=aux;
            }
        }
    }

    return arreglo.at(arreglo.size()-1);
}
